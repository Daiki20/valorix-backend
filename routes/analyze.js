...
ШАГ 1 — СБОР ДАННЫХ: ${cfg.liveItems}

ШАГ 2 — АНАЛИЗ: Оцени ход матча, динамику и вероятный исход.

ШАГ 3 — ВЫВОД: Ответь СТРОГО в JSON (без markdown, без текста вне JSON):
{
  "verdict": "конкретный вердикт с учётом текущего счёта и хода матча",
  "summary": "3-4 предложения с конкретными фактами — счёт, голы, кто доминирует",
  "confidence": число 50-85,
  "risk": "low | medium | high",
  "fairOdds": "справедливый коэф прямо сейчас",
  "bookOdds": null,
  "value": 0,
  "reasons": ["конкретный факт 1", "факт 2", "факт 3"],
  "extraBets": [
...
Дополнительные ставки для анализа: ${cfg.betTypes}`
  ...
  try {
    const raw = await callOpenAIWithWebSearch([
      { role: 'system', content: 'Ты профессиональный беттинг-аналитик. Всегда ищи актуальные данные в интернете перед ответом. Никогда не придумывай статистику — если данных нет, укажи в dataWarning. Отвечай ТОЛЬКО валидным JSON без markdown.' },
