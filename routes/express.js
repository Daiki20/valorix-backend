...
- Для тоталов (ТБ/ТМ), фор и других рынков — оцени реалистично как у топ-букмекеров`
    ...
    const betTypes = isHigh
      ? `  * Двойной шанс "1X" / "X2" (коэф ~1.25–1.60) — если есть явный фаворит
...
- Ссылайся на конкретные цифры: "в последних X встречах ...", "забивает X.XX голов/игру"`
  ...
  const betTypes = isHigh
    ? `  * Двойной шанс "1X" / "X2" (коэф ~1.25–1.60) — если есть явный фаворит
  * Тотал "ТБ 2.5" / "ТМ 2.5" (коэф ~1.50–1.80) — если голевая статистика очевидна
  * Победа фаворита "П1" / "П2" — только если коэф ≤ 1.80 и явное превосходство
  ЗАПРЕЩЕНО: коэф > 1.80`
